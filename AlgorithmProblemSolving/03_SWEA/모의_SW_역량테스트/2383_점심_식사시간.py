...
def simulate(choices):
    ...
    for p in range(P):
        pi, pj = ppl_lst[p]
        si, sj = stair_lst[choices[p]]
        at_lst.append(abs(pi - si) + abs(pj - sj))
    ...
    while True:
        ...
        for p in range(P):  # 계단 비우기
            st = choices[p]
            if at_lst[p] == -k_lst[st]:  # 도착
                cnts[st] -= 1
                at_lst[p] = -INF

        for p in range(P):  # 이동
            st = choices[p]
            if at_lst[p] > 0:  # 계단까지 이동
                at_lst[p] -= 1
            elif at_lst[p] == 0:  # 계단에 진입
                if 3 > cnts[st]:  # 진입 가능
                    at_lst[p] -= 1
                    cnts[st] += 1
            else:  # 계단 내려가기
