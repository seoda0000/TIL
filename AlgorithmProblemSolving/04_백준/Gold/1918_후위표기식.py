...
before_dic = {'(':4, '+':1, '-':1, '*':2, '/':2}  # 우선순위 딕셔너리
after_dic = {'(':0, '+':1, '-':1, '*':2, '/':2}
...
ans = ''
for s in string:
    if s.isalpha(): # 만약 알파벳이면 ans에 추가
        ans += s
    elif s == ')':  # 닫는 괄호면 여는 괄호가 나올 때까지 stk pop
        while stk[-1] != '(':
            ans += stk.pop()
        stk.pop()
    else:
        if not stk or before_dic[s] > after_dic[stk[-1]]:  # stk 마지막 값보다 우선순위가 클 경우 push
            stk.append(s)
        else:
            while stk and before_dic[s] <= after_dic[stk[-1]]:  # 아닐 경우 조건 충족할 때까지 stk pop
                ans += stk.pop()
            stk.append(s)
            
while stk:          # 마지막 stk 비워주기
    ans += stk.pop()
print(ans)
